"""
Metrics computation for next-location prediction.
"""
...
def calculate_correct_total_prediction(logits, true_y):
    """
    Calculate metrics including top-k accuracy, MRR, and NDCG.
    
    Args:
        logits: Model output logits [batch_size, num_locations]
        true_y: True labels [batch_size]
    
    Returns:
        result_array: [correct@1, correct@3, correct@5, correct@10, rr, ndcg, f1, total]
        true_y_cpu: True labels on CPU
    ...
    # F1 score - compute after getting predictions
    f1 = 0.0  # Placeholder, will compute properly if needed
    result_ls.append(f1)
...
def get_performance_dict(return_dict):
    """
    Convert raw metrics to performance percentages.
    
    Args:
        return_dict: Dictionary with keys: correct@1, correct@3, correct@5, 
                     correct@10, rr, ndcg, f1, total
    
    Returns:
        Performance dictionary with computed percentages
    """
    perf = {
